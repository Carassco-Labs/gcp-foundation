...
`gcp-foundation` implements a modern GitOps-aligned Continuous Integration and Continuous Delivery (CI/CD) pipeline. 
...
- Published to GCP Artifact Registry (`us-docker.pkg.dev/carassco-prod/backend/gcp-foundation:v1.2.0`).
...
gcloud run revisions list --service=gcp-foundation --region=us-central1

# Instantly revert 100% of traffic to previous known healthy revision
gcloud run services update-traffic gcp-foundation \
  --region=us-central1 \
  --to-revisions=gcp-foundation-00042-xyz=100
```
