...
> **Comprehensive guide to environment configuration, GCP Secret Manager integration, hierarchy resolution, and multi-environment setup for `gcp-foundation`.**
...
| `APP_NAME` | `str` | `gcp-foundation` | Service identifier used in log correlation and metrics |
...
| :--- | :--- | :--- |
| `secret_key` | `SECRET_KEY` | `gcp-foundation-secret-key` |
| `database_url` | `DATABASE_URL` | `gcp-foundation-db-url` |
| `api_key_gemini` | `GEMINI_API_KEY` | `carassco-gemini-api-key` |
| `redis_password` | `REDIS_PASSWORD` | `gcp-foundation-redis-password` |
